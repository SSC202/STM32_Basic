...
> 对于 PC 端运行的程序而言，通常的代码运行流程如下：
>
> ![NULL](./assets/picture_32.jpg)
>
> > - 预处理：删除所有的注释，宏扩展，文件包含；
> > - 编译：使用编译器软件将预处理后的高级语言代码编译为汇编代码；
> > - 汇编：使用汇编程序将汇编代码汇编形成机器码 (01)；
> > - 链接：将一些预定义的机器码链接到汇编形成的机器码中，最后形成计算机可执行的可执行程序 (01)。

对于单片机而言，片上不存在编译器和汇编程序，自然也无法直接在单片机上完成程序的编写，因此必须在 PC 端编写好源代码后进行编译和汇编形成机器码，再将机器码下载（称为烧录）到单片机中，单片机上电就能直接执行机器码，这个过程称为交叉编译。

针对 STM32 ，常见的交叉编译链是 AC5/6（Keil）和 arm-none-eabi-gcc 编译链。烧录则使用 STLink，Jlink 和 OpenOCD 烧录程序进行烧录。

STM32 的集成开发环境（包含编写代码，编译，烧录的软件）：Clion，Keil，STM32CubeIDE，....

STM32CubeMX 是用于 32 位 ARM Cortex STM32 微控制器的图形工具。它允许对 STM32 微控制器和微处理器进行非常简单的配置，**生成初始化 C 代码**（不是开发环境，只是配置工具）。

## 2. Windows 环境配置

> 配置前注意：**安装路径不能有中文（包括电脑用户名）！！！**

### 1. Keil + STM32CubeMX 环境配置
...
### 3. STM32CubeMX + VSCode + EIDE 环境配置
...
基本上使用 arm-none-eabi-gcc 编译链，配置思路大同小异，此处略。
