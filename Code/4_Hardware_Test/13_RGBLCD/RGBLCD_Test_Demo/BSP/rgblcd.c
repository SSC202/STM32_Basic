...
static void delay_us(uint32_t us)
{
    ...
    __HAL_TIM_SET_AUTORELOAD(&htim7, us);
...
/********************************** LTDC 初始化段 ***********************************************/
#include "ltdc.h"
#define LCD_LTDC_LAYER_FB_ADDR ((uint32_t)0xC0000000)   // SDRAM存储地址
/********************************* LCD  显示驱动段 **********************************************/
...
static inline void LCD_DMA2D_Fill(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye, uint16_t color)
{
    ...
    DMA2D->CR &= ~DMA2D_CR_START;                                 /* 停止DMA2D */
    DMA2D->OOR  = LCD_LCD_RAW_WIDTH - (xe - xs + 1);              /* 行偏移，即跳过的像素 */
