...
void UDP_Echo_Init(void)
{
    ...
    udpecho_pcb = udp_new();                                     // 新建控制块
    udp_bind(udpecho_pcb, IP_ADDR_ANY, UDP_ECHO_PORT);           // 绑定端口号
    udp_recv(udpecho_pcb, udp_demo_callback, (void *)st_buffer); // 接收数据回调函数
}
